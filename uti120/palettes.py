...
def _make_iron_palette() -> np.ndarray:
...
def _make_rainbow_palette() -> np.ndarray:
    lut = np.zeros((256, 1, 3), dtype=np.uint8)
    for i in range(256):
...
def _make_whitehot_palette() -> np.ndarray:
    lut = np.zeros((256, 1, 3), dtype=np.uint8)
    for i in range(256):
        lut[i, 0] = [i, i, i]
    return lut
...
def _make_blackhot_palette() -> np.ndarray:
    lut = np.zeros((256, 1, 3), dtype=np.uint8)
    for i in range(256):
        v = 255 - i
        lut[i, 0] = [v, v, v]
...
PALETTES = {
    "Iron": _make_iron_palette(),
    "Rainbow": _make_rainbow_palette(),
    "Whitehot": _make_whitehot_palette(),
    "Blackhot": _make_blackhot_palette(),
    "Jet": None,
    "Inferno": None,
}


def apply_palette(normalized_u8: np.ndarray, name: str) -> np.ndarray:
    """Apply a color palette to a normalized uint8 grayscale image."""
    lut = PALETTES.get(name)
    if name == "Jet":
